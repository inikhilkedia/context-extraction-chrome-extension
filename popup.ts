...
// popup.ts - Logic for the extension popup

document.addEventListener("DOMContentLoaded", () => {
	const copyButton = document.getElementById("copyButton") as HTMLButtonElement;
	const extractedTextElement = document.getElementById("extractedText") as HTMLTextAreaElement;
	const multiTabButton = document.getElementById("multiTabButton") as HTMLButtonElement;
	...
	async function extractFromAllTabs() {
		...
		try {
			...
			const results = await Promise.all(
				tabs.map(tab => {
					return new Promise<{ tab: chrome.tabs.Tab; text: string | null }>(resolve => {
						if (!tab.id) return resolve({ tab, text: null });
							...
							() => {
								...
								const handler = (msg: any, sender: chrome.runtime.MessageSender) => {
									if (
										...
										msg.type === "TEXT_EXTRACTION_RESULT"
									) {
	...
	chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
		if (request.type === "TEXT_EXTRACTION_RESULT") {
			showLoading(false);
			if (request.extractedText !== undefined && request.extractedText !== null) {
	...
	// Automatically trigger extraction when the popup DOM is loaded
	triggerExtraction();
});
